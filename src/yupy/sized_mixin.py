...
class SizedMixin(ISchema[_ST]):
    def length(self, limit: int, message: ErrorMessage = locale["length"]) -> 'ISchema[_ST]':
    ...
    def min(self, limit: int, message: ErrorMessage = locale["min"]) -> 'ISchema[_ST]':
        def _(x: Sized) -> None:  # Use Sized instead of Iterable
            if len(x) < limit:
    ...
    def max(self, limit: int, message: ErrorMessage = locale["max"]) -> 'ISchema[_ST]':
        def _(x: Sized) -> None:  # Use Sized instead of Iterable
            if len(x) > limit:
