...
def test_mapping_schema_shape_success():
    ...
    user_schema = MappingSchema().shape({"name": name_schema, "age": age_schema})
...
def test_mapping_schema_strict_true_unknown_keys_fails():
    schema = (
        MappingSchema().shape({"name": StringSchema()}).strict(True)
    )  # Explicitly set strict to True
    ...
    assert (
        ...
        == "Object contains unknown keys: ['extra']"
    )


def test_mapping_schema_strict_false_unknown_keys_succeeds():
    schema = (
        MappingSchema().shape({"name": StringSchema()}).strict(False)
...
def test_mapping_schema_validate_simple_success():
    schema = MappingSchema().shape(
        {"name": StringSchema(), "age": NumberSchema().integer()}
    )
...
def test_mapping_schema_validate_nested_success():
    address_schema = MappingSchema().shape(
        {"street": StringSchema(), "zip": StringSchema().length(5)}
    )
    user_schema = MappingSchema().shape(
        {"name": StringSchema(), "address": address_schema}
    )
...
def test_mapping_schema_validate_with_nullable_field_present():
    schema = MappingSchema().shape(
        {"name": StringSchema(), "email": StringSchema().nullable()}
    )
...
def test_mapping_schema_validate_missing_required_field_abort_early():
    schema = MappingSchema().shape(
        {"name": StringSchema(), "age": SchemaRequiredAdapter(NumberSchema().integer())}
    ...
    assert (
        excinfo.value.invalid_value is _REQUIRED_UNDEFINED_
    )  # The invalid value for a missing required field is None
...
def test_mapping_schema_validate_invalid_field_type_abort_early():
    ...
    assert excinfo.value.path == "~/age"
    assert (
        excinfo.value.invalid_value == "thirty"
    )  # The invalid value should be 'thirty', not the whole dict
    assert (
        "Value is not of type " in excinfo.value.constraint.format_message
    )  # Order of int, float might vary
...
def test_mapping_schema_validate_nested_error_abort_early():
    ...
    assert (
        excinfo.value.path == "~/address/zip"
    )  # Path should be the specific failing field
...
def test_mapping_schema_validate_multiple_errors_collect_all():
    ...
    assert (
        all_errors[0].constraint.format_message == "Invalid mapping"
...
def test_mapping_schema_validate_nested_and_multiple_errors_collect_all():
    ...
    address_object_error = next(
        e for e in all_errors if e.path == "~/address"
    )  # Fully qualified path
    assert address_object_error.constraint.type == "mapping"

    # Check nested errors within address (order by path for consistency)
    # Paths should be fully qualified in the collected errors
    nested_address_errors = sorted(
        [e for e in all_errors if e.path.startswith("~/address")], key=lambda x: x.path
    )

    street_error = next(
        e for e in nested_address_errors if e.path == "~/address/street"
    )
